...
#include <fstream> // Эта библиотека позволяет нам работать с внешними файлами
...
int main() { // С функции main начинается каждая программа в языке C++
    ...
    { // Переменные
        ...
        // С помощью sizeof() можно узнать размер типа или переменной в байтах:
            int n = 0;
            int size1 = sizeof(int); // 4 байта
            int size2 = sizeof(n); // 4 байта

        // Ключевое слово auto позволяет не указывать тип переменной явно, говоря компилятору,
        // чтобы он сам определил фактический тип переменной, на основе типа инициализируемого значения.
            auto i = 42; // i - типа int
            auto l = 42ll; // l - типа long long
    ...
    {  // Определения типов
        ...
        /* Объявление typedef представляет имя, которое в пределах области становится синонимом типа, заданного частью объявления типа.
         * Объявления typedef можно использовать для создания более коротких или более значимых имен для типов,
         * уже определенных языком или для объявленных типов.
         * Имена typedef позволяют инкапсулировать детали реализации, которые могут измениться.
         * Объявления не вводят новые типы, они вводят новые имена для существующих типов.

         * Имена, объявленные с использованием typedef пространства имен, занимают то же пространство имен, что и другие идентификаторы.
         * Поэтому они не могут использовать тот же идентификатор, что и ранее объявленное имя, за исключением объявления типа класса. */
            typedef unsigned long UL;
            int UL; // Ошибка

        // Однако можно скрыть typedef-имя в локальной области видимости, переопределив его как название переменной:

            typedef unsigned long UL;
            int main() {
                int UL; // Далее UL будет использоваться как имя переменной, а не типа
                UL num; // Ошибка
            } // Вне main() UL снова будет виден программой как название типа

    }

    { // Перечисления

        /* Перечисления (enum) представляют еще один способ определения своих типов.
         * Их отличительной особенностью является то, что они содержат набор числовых констант. */
        ...
        /* Для определения перечисления применяется ключевое слово enum, после которого идет название перечисления.
         * Затем в фигурных скобках идет перечисление констант через запятую.
        ...
        // Если нас не устраивают значения по умолчанию для констант, то мы можем явным образом задать значения.
        // Например, установить начальное значение:
            enum seasons {
                winter = 1,
                spring, //2
                summer, //3
    ...
    { // Операторы ввода и вывода
        ...
        // cout - оператор вывода
            ...
            // Можно вывести и переменные:
                int integer = 5;
                double dfloat = 1.23;
                cout << integer << dfloat << endl;
    ...
    { // Массивы
        ...
        // Также можно использовать sizeof(), чтобы узнать размер массива в байтах:
            int size = sizeof(arr); // size = 20, т.к. 5 элементов по 4 байта
    ...
    { // Векторы (работает при подключении библиотеки <vector> и при использовании пространства имён std)
        ...
        /* В векторах для перебора элементов используются не индексы, а итераторы. Итераторы обеспечивают доступ к элементам контейнера.
         * С помощью итераторов очень удобно перебирать элементы. Итератор описывается типом iterator.
         * Но для каждого контейнера конкретный тип итератора будет отличаться.
         * Так,  итератор контейнера vector<int> представляет тип vector<int>::iterator и так далее. */
        ...
        // Можно поместить элемент или массив в вектор, используя функцию insert():
             ...
             * Также существует функция end(), которая возвращает итератор,
             * указывающий на следующую позицию после последнего элемента, то есть по сути на конец контейнера. */
    ...
    { // Циклы
        ...
        // Цикл на основе диапазона (range-based):

            /* В C++, по сравнению с Си, теперь можно использовать оператор for для создания циклов,
             * которые должны выполняться через весь диапазон, который определяется как любой, через который можно выполнить итерацию.
             * Данные циклы автоматически распознают массивы и контейнеры (к примеру, тот же вектор).
             * С другими конструкциями они будут работать, если у них заданы итератор-функции begin() и end().
             * Предпочтительно в данном случае использовать ключевое слово auto для нахождения элемента в массиве/контейнере. */
                int arr[3] = {10, 20, 35};
                for(auto i : arr) { cout << i << " "; } // Выведет "10 20 35 "
    ...
    { // Функции
        ...
        // Чтобы создать функцию, необходимо прописать её до функции main().
            // Сначала необходимо прописать тип данных, который будет возвращён самой функцией.
                ...
                // Важное примечание: функция не может возвращать auto.
    ...
    { // Пространства имён
        ...
        // Конфликт имён
            ...
            /* Конфликт имен возникает, когда два одинаковых идентификатора находятся в одной области видимости,
             * и компилятор не может понять, какой из этих двух следует использовать в конкретной ситуации.
             * Компилятор выдаст ошибку, так как у них недостаточно информации, чтобы решить эту неоднозначность.
             ...
             * следовательно, увеличивается и вероятность возникновения конфликтов имен. */
                ...
                int main() {
                    cout << doOperation(5, 4); // Непонятно, какую из версий doOperation вызывать
                }
        ...
        // Пространство имён
            ...
            // Чтобы избежать конфликта имён, нужно объявлять собственные пространства имен через ключевое слово namespace.
            ...
            /* Теперь, чтобы получить доступ к переменным и функциям внутри пространства имён,
             * достаточно использовать название необходимого пространства
             * вместе с оператором разрешения области видимости (::) и требуемым идентификатором. */

                int main() {
                    cout << plus::doOperation(5, 4); // Выведется 9
                    cout << minus::doOperation(5, 4); // Выведется 1
                }

            /* Допускается объявление пространств имен в нескольких местах
             * (либо в нескольких файлах, либо в нескольких местах внутри одного файла).
             * Всё, что находится внутри одного блока имен, считается частью только этого блока. */

                namespace DoMath {
                    int add(int x, int y) { // Функция add() является частью пространства имен DoMath
                        return x + y;
                    }
                }

                namespace DoMath {
                    int subtract(int x, int y) { // Функция subtract() является частью пространства имен DoMath
                        return x - y;
                    }
                }

                int main() {
                    cout << DoMath::add(5, 4); // Выведет 9
                    cout << DoMath::add(5, 4); // Выведет 1
                } // Всё работает, несмотря на то, что мы объявили пространство имён DoMath дважды
        ...
        // Псевдонимы и вложенные пространства имён
            ...
            // Одни пространства имен могут быть вложены в другие пространства имен. Например:
                ...
                int main() {
                    cout << Boo::Doo:x; // Выведет 5
                }

            /* Так как не всегда удобно обращаться через несколько вложенных пространств,
             * можно создавать псевдонимы для пространства имён: */
                namespace Foo = Boo::Doo;

                int main() {
                    cout << Foo:x; // Выведет 5
                }

        // Ключевое слово using

            /* При использовании стандартной библиотеки C++ постоянное добавление std::
             * к используемым объектам может быть несколько утомительным. За этим и придумали ключевое слово using. */

            // Using-объявления

                // Одной из альтернатив является использование "using-объявления":
                    int main() {
                        using std::cout; // "using-объявление" сообщает компилятору, что cout следует обрабатывать, как std::cout
                        cout << "Hi" << std::endl; // и никакого префикса std:: перед cout уже здесь не нужно!
                    }

                    /* Строка using std::cout; сообщает компилятору, что мы будем использовать объект cout из пространства имен std.
                     * И каждый раз, когда компилятор будет сталкиваться с cout, он будет понимать, что это std::cout. */

            // Using-директивы

                // Второй альтернативой является использование "using-директивы":
                    int main() {
                        using namespace std; // "using-директива" сообщает компилятору, что мы используем все объекты из пространства имен std
                        cout << "Hi" << endl; // так что никакого префикса std:: здесь уже не нужно!
                    }

                /* Много разработчиков спорят насчет использования «using-директивы».
                 * Так как с её помощью мы подключаем ВСЕ имена из пространства имен std,
                 * то вероятность возникновения конфликтов имен значительно возрастает
                 ...
                 * так что, если компилятор найдет имя, которое не сможет распознать, он будет проверять его наличие в пространстве имен std. */
                ...
                // Пример конфликта с "using-директивой":
                    ...
                    int main() {
                        ...
                        cout << "No"; // Компилятор не может понять, какой из cout ему использовать
                    }

                    // Если бы мы использовали "using-объявление", то программа была бы без ошибок:
                        int cout() { return 0 };
                        int main() {
                            using std::cout;
                            cout << "Yes"; // Выведется "Yes"
                        }

            // Области видимости "using-объявлений" и "using-директив"

                /* Если «using-объявление» или «using-директива» используются в блоке,
                 * то они применяются только внутри этого блока (по обычным правилам локальной области видимости).
                 * Это хорошо, поскольку уменьшает масштабы возникновения конфликтов имен до отдельных блоков.
                 * Однако многие начинающие программисты пишут «using-директиву» в глобальной области видимости
                 * (вне функции main() или вообще вне любых функций).
                 * Этим они вытаскивают все имена из пространства имен std напрямую в глобальную область видимости,
                 * значительно увеличивая вероятность возникновения конфликтов имен. А это уже не хорошо.

                 * Правило: Никогда не используйте using-стейтменты вне тела функций.

                 * Как только было объявлено одно using-объявление или using-директива,
                 * его невозможно отменить или заменить другим объявлением/директивой в пределах области видимости, в которой он был объявлен. */

                    int main() {
                        using namespace Boo;
                        // ...
                        return 0
                    } // Действие using namespace Boo кончится здесь

                // Чтобы ограничить область применения using-объявления/директивы, используем правило локальной области видимости:
                    ...
                    int main() {
...

                        {
                            using namespace Doo;
                            // ...
                        } // Действие using namespace Doo кончится здесь
    ...
    { // Объектно-ориентированное программирование
        ...
        { // Классы/структуры и объекты
            ...
            /* Классы подобны структурам из Си, но в то же время их функционал обширнее, чем у структур.
            ...
            // Классы, как и структуры в Си, прописываются до функции main. Также после каждого класса надо ставить точку с запятой (;).

            // По умолчанию поля и методы класса являются приватными.
            ...
            // Пример класса:
                ...
                public:
                    ...
                    void print() const { // const после метода означает, что мы не будем изменять *this
                ...
                /* Структура в C++ не является структурой из Си (!!!).
                 * Фактически, структура в C++ - это тот же класс, только по умолчанию поля и методы структуры являются публичными */
        ...
        { // Конструкторы и деструкторы
            ...
            // Пример:
                ...
                public:
                    ...
                    // Ключевое слово explicit ставится перед объявлением конструктора с одним параметром
                    // (или с большим числом параметров, но когда все параметры, начиная со второго, имеют значения по умолчанию)
        ...
        { // Наследование классов
            ...
            // Типы наследования
                ...
                /* В C++ есть несколько типов наследования:
                    ...
                    class Motorcycle: private Car {};
                    int main() {
                        Motorcycle mc;
                        mc.printHi(); // Так сделать нельзя
                    } /* Для базового класса Car, уровень доступа к данным не изменяется,
                       * но поскольку производный класс Motorcycle наследует данные как приватные, данные становятся приватными для класса Motorcycle. */

                // По умолчанию в классах наследование приватное, в структурах - публичное:

                    class Car {};
                    class Truck: Car {}; // Все унаследованные данные из Car будут приватными в Truck

                    struct Device {};
                    struct Phone: Device {}; // Все унаследованные данные из Device будут публичными в Phone
        ...
        { // Виртуальные функции
            ...
            // Чисто виртуальные функции
                ...
                // Интерфейс
                    ...
                    class SimpleBicycle: public IFBicycle { // Говорим: не наследуемся от интерфейса, а реализовываем интерфейс
                    public:
                    ...
                    class SportBicycle: public IFBicycle {
                    public:
                        void TwistWheel() override {
        ...
        { // Static в классах
            ...
            // Статические поля класса:
                ...
                // т.е. если мы создали три объекта класса B, то конструктор статического поля класса будет вызван только один раз.
                    ...
                    class B {
                    public:
                        B() { cout << "Constructor B" << count++ << endl; }
                        ~B() { cout << "Destructor B" << --count << endl; }
                    private:
        ...
        { // Перегрузка операторов
            ...
            // Операторы вывода/ввода
                ...
                // Сразу следует отметить, что операторы ввода и вывода не должны быть членами в классе, а определяются вне класса как обычные функции.

                // Оператор <<

                    /* Обычно первый параметр оператора << представляет ссылку на неконстантный объект ostream.
                     * Данный объект не должен представлять константу, так как запись в поток изменяет его состояние.
                     * Причем параметр представляет именно ссылку, так как нельзя копировать объект класса ostream.
                ...
                // Оператор >>
                    ...
                    /* Первый параметр оператора >>, как правило, представляет ссылку на объект istream, с которого осуществляется чтение.
                     * Второй параметр представляет ссылку на неконстантный объект, в который надо считать данные.
    ...
    { // Работа с файлами
         ...
         * В целом функционал этой библиотеки схож с функционалом библиотеки <iostream>, который работает с консолью, поскольку консоль это тоже файл.
         * В этой библиотеке есть три класса, которые отвечают за работу с файлами:
            * ifstream - класс для чтения из файла;
            * ofstream - класс для записи в файл;
            * fstream - универсальный класс, сочетающий в себе функционал ifstream и ofstream.

         * Наиболее используемые операции:
            * Операторы перенаправления ввода\вывода – << и >>;
            * Метод чтения строк getline();
        ...
        { // Запись в файл (ofstream)
            ...
            // Чтобы записать что-либо в файл, надо его сначала открыть по какому-либо пути (если не указать путь, open() будет искать файл в папке проекта):
                ofstream fout;
                fout.open("myFile.txt"); // Если такого файла изначально нет, он создастся автоматически

            // Также можно передать переменные типа const char* или string в open() и открывать файл с помощью конструктора:
                string path = "myFile.txt";
                fout.open(path);

                string path2;
                cin >> path2; // Вводим путь с консоли
                ofstream fout2(path2);

                /* В данном случае, так как мы не указали дополнительных режимов открытия файла, файл будет перезаписан полностью.
                 * Чтобы избежать этого, добавим режим работы app (от слова append - добавить). */
                    fout.path(path, ios::app); // Либо ofstream::app, либо fstream::app.
                    // Для класса ofstream автоматически всегда выставлен режим записи данных ios::out.

            // С помощью метода is_open() можно проверять, открыт ли файл или нет:
                bool t = fout.is_open(); // t = true, т.к. мы ранее открыли файл.
                // Желательно всегда делать подобную проверку! А также сочетать их с исключениями, указывая причину, почему файл не открылся.

            // Чтобы записать данные в файл, достаточно использовать знакомый нам оператор перенаправления вывода <<:
                fout << "Запишем данные" << endl; // Не принципиально, какой тип данных записывать файл, главное, чтобы для него был перегружен оператор <<.
                // Для перевода на новую строку здесь так же используется оператор endl.

            // Также существует метод write(), который используется в бинарных файлах для записи блока памяти (массива байт) какой он есть (не текстом).
                const char* buf = "hello";
                fout.write(buf, strlen(buf));

                int k = 12345;
                fout.write((char*)&k, sizeof(k)); // Т.к. передаём массив байтов, преобразуем всё в char* (массив char).

            // В конце программы закрываем файл, который до этого открывали для работы с ним:
                fout.close(); // Делать это предпочтительно, хоть и не обязательно (при деструкторе класса файл тоже закроется).

        }

        { // Чтение из файла (ifstream)

            // Чтобы считать что-либо из файла, надо его для начала открыть. Допустим, откроем тот, в который мы ранее записывали данные:
                string path("myFile.txt");
                ifstream fin;
                fin.open(path); // Для класса ifstream автоматически всегда выставлен режим чтения данных ios::in.
                // Если такого файла изначально нет, он не сможет открыться (!!!).

            // Так как мы можем открывать файл и при чтении из файла, нам так же доступен метод is_open():
                bool t = fin.is_open(); // t = true

            // Для чтения из файла посимвольно используется метод get():
                char ch;
                while(fin.get(ch)) cout << ch; // Метод get() возвращает true или false в зависимости от того, записал ли он что-либо в ch или нет.

            // Оператор считывания >> читает данные в файле до первого пробела или перехода на новую строку.
                int k, string s, double f;
                fin >> k >> s >> f;

            // С помощью этого оператора можно считывать слова в файле:
                string str;
                while(!fin.eof()) { // Метод eof() возвращает true, если мы достигли конца файла
                    str = "";
                    fin >> str;
                    cout << str << endl;
                }

            // Для чтения из файла построчно используется метод getline():
                const char buf[10];
                fin.getline(buf, 10, ';'); // Последний аргумент - символ-разделитель (причём он не войдёт в строку). По умолчанию - переход на новую строку.

            // Для строк типа string рекомендуется использовать переопределённую версию getline() в виде функции:
                string str;
                while(getline(fin, str)) cout << str << endl;

            // Аналогично методу write() существует метод read(), применяющийся для чтения бинарных файлов.
                int n = 10, const char* buf = new char[n];
                fout.read(buf, n); // Запишет n байтов в str

            // В конце программы, как и при записи в файл, закрываем сам файл:
                fin.close();
        ...
        { // Чтение и запись в файл (fstream)
            ...
            string path = "myFile.txt";
            fstream fs(path, ios::in | ios::out | ios::app); // Указываем через логическое ИЛИ, что будем использовать файл и для чтения, и для записи, и для дозаписи

            // Чтобы записать русские символы, надо подключить библиотеку <Windows.h> и сменить кодировку консоли, затем после записи необходимо вернуть её обратно:
                #include <Windows.h>
                string msg;
                SetConsoleCP(1251); // Кодировка для русских символов
                cin >> msg;
                fs << msg << endl;
                SetConsoleCP(866); // Кодировка по умолчанию
        }
        ...
        { // Запись и чтение объекта класса в файл
            ...
            // Запись
                ...
                int main() {
                    Point point(5, 10, 15);
                    ofstream fout(path);
                    fout.write((char*)&point, sizeof(Point)); // Записали побитово данные об объекте
                    fout.close();
                }
            ...
            // Чтение
                ...
                int main() {
                    ifstream fout(path);
                    Point point;
                    fout.read((char*)&point, sizeof(Point)); // Прочитали данные о первом объекте и записали их
                    // Чтобы получить все объекты, получим их с помощью while():
                    while(fout.read((char*)&point, sizeof(Point))) {
                        point.Print();
                    }
                    fout.close();
                }
        ...
        { // Использование перегруженных операторов вывода и ввода

            /* Определив операторы ввода и выводы, мы можем их использовать также и для чтения и записи файла,
             * так как класс ofstream наследуется от класса ostream. */

            // Используем выше описанный класс Point:
                ...
                int main() {
                    vector<Point> points = { Point(1, 2, 3), Point(10, 20, 15), Point(20, 0, 121) };
                    fstream fs("myFile.txt");
                    for(auto point: points) { fs << point; } // Записали в файл данные, как мы сами хотели

                    vector<Point> newPoints;
                    Person p;
                    while(fs >> p) newPoint.push_back(p); // Прочитали в файл данные, которые записали ранее, и записали их в новые объекты
                }
    ...
    { // Спецификатор constexpr
        ...
        // С помощью спецификатора constexpr можно создавать переменные, функции и даже объекты, которые будут рассчитаны на этапе компиляции.
            constexpr int sum (int a, int b) { return a + b; }
            void func() {
                constexpr int c = sum (5, 12); // Значение переменной будет посчитано на этапе компиляции
            }

        /* Ключевое слово constexpr перед функцией означает, что если значения параметров возможно посчитать на этапе компиляции,
         * то возвращаемое значение также должно посчитаться на этапе компиляции.
         * Если значение хотя бы одного параметра будет неизвестно на этапе компиляции,
         * то функция будет запущена уже во время выполнения программы (а не будет выведена ошибка компиляции). */

        // Ключевое слово constexpr перед переменной означает создание константы. Причем присваиваемое значение должно быть известно на этапе компиляции. Пример:
            int sum (int a, int b) { return a + b; }
            constexpr int new_sum (int a, int b) { return a + b; }
            void func() {
                constexpr int a1 = new_sum(5, 12); // ОК: constexpr-переменная
                constexpr int a2 = sum(5, 12); // Ошибка: функция sum не является constexpr-выражением
        ...
        /* Ограничения constexpr до версии C++14:
         ...
         * constexpr-функция должна удовлетворять следующим условиям:
            * Она не может быть виртуальной;
            * Она должна возвращать литеральный тип (void вернуть нельзя); // Пометка: с версии C++14 void также является литеральным типом
            * Все параметры должны иметь литеральный тип;
            * Тело функции должно содержать только следующее:
                * static_assert (проверка утверждения на уровне компиляции);
                * typedef или using, которые объявляют все типы, кроме классов и перечислений;
                * using для указания видимости имен или пространств имен;
                * Ровно один return, который может содержать только литералы или constexpr-переменные и constexpr-функции.

         * В constexpr-конструкторе все нестатические члены класса и члены базовых классов должны быть инициализированы каким-либо образом
         * (в конструкторе, используя списки иницилизации или иницилизацией членов класса при объявлении),
         * причем присвоенные им выражения должны содержать только литералы или constexpr-переменные и constexpr-функции.

         * Проще говоря, в функциях нельзя инициализировать переменные, создавать циклы и конструкции if-else.
         * Зато не запрещается использовать рекурсию заместо циклов и тернарный оператор (? :) заместо if-else. */

        /* Изменения в C++14:

         * В C++14 void также будет литеральным типом, и теперь можно будет создавать функции, которые, например, будут проверять значения параметров на правильность.
         ...
         * Теперь тело constexpr-функции может содержать любые конструкции, кроме:
            ...
            * Ключевого слова goto (его не используйте в любом случае);
            * Определения переменных нелитерального типа или static-переменных. Все переменные должны инициализироваться при определении.
    ...
    { // Шаблоны
        ...
        { // Шаблоны функций
             ...
             * Другими словами, шаблон функции (шаблонная функция, обобщённая функция)
             * представляет собой семейство разных функций (или описание алгоритма).
             * По описанию шаблон функции похож на обычную функцию,
             * разница в том, что некоторые элементы не определены (типы, константы) и являются параметризованными. */

            // Рассмотрим случай определения максимального элемента из двух. В случае целых и действительных чисел придется написать 2 функции.
                ...
                /* Поскольку C++ требует, чтобы мы указывали типы наших переменных,
                 * нам приходится записывать несколько версий одной и той же функции, где единственное, что меняется — это тип параметров.
                 * А это, в свою очередь, головная боль для программистов, так как поддерживать такой код непросто как по затраченным усилиям, так и по времени.
                 * И самое важное то, что это нарушает одну из концепций эффективного программирования — сократить до минимума дублирование кода.

                 * В этом случае, если алгоритм является общим для типов, с которыми приходится работать,
                 * можно определить шаблон функции. Принцип, в общем случае, будет следующим:
                    * 1. Берётся реализация функции для какого-то типа.
                    * 2. Приписывается заголовок template<typename Type> (или template<class Type>),
                         что означает, что в алгоритме используется какой-то абстрактный тип Type.
                    * 3. В реализации функции имя типа заменяется на Type. */

                // Для функции max() получится следующее:
                    template <typename T> // объявление параметра шаблона функции
                    T max(T a, T b) {
                        return (a > b) ? a : b;
                    }

                /* Самым интересным является тот факт, что пока нет вызова функции max(),
                 * при компиляции она в бинарном коде не создается (не инстанцируется), в отличие от того, если бы мы отдельно задавали функции для отдельных типов.
                 * А если объявить группу вызовов функции с переменными различных типов, то для каждого компилятор создаст свою реализацию на основе шаблона. */

                /* Вызов шаблонной функции, по общему счёту, эквивалентен вызову обыкновенной функции.
                 * В этом случае компилятор определит, какой тип использовать вместо Type, на основании типа фактических параметров.
                 * Но если подставляемые параметры окажутся разных типов, то компилятор не сможет вывести (инстанцировать шаблон) реализацию шаблона. */

                int main() { // Используем выше написанную функцию max()
                    cout << max(1, 2); // Выведет 2
                    cout << max(1.1, 2.2); // Выведет 2.2
                    cout << max(1, 2.2) // Ошибка
                }

                // Данная ошибка решается указанием конкретного типа при вызове функции:
                    int main() {
                        cout << max<double>(1, 2.2); // Выведет 2.2
                    } // Фактически, мы просто заставили компилятор конвертировать 1 в 1.f

            // Специализация шаблона функции

                /* В некоторых случаях шаблон функции является неэффективным или неправильным для определенного типа.
                 * В этом случае можно специализировать шаблон, — то есть написать реализацию для данного типа.
                 * Например, в случае со строками можно потребовать, чтобы функция сравнивала только количество символов.
                 * В случае специализации шаблона функции тип, для которого уточняется шаблон в параметре не указывается. */

                // Специализируем выше написанный шаблон для функции max():
                    template<>
                    string max(string a, string b) {
                        return (a.size() > b.size()) ? a : b;
                    }

                /* Специализация шаблона для конкретных типов делается опять же из соображения экономичности:
                 * если эта версия шаблона функции в коде не используется, то она не будет включена в бинарный код. */

        }

        { // Шаблоны классов

            /* Шаблоны классов -– обобщенное описание пользовательского типа,
             * в котором могут быть параметризованы атрибуты и операции типа. Представляют собой конструкции,
             * по которым могут быть сгенерированы действительные классы путём подстановки вместо параметров конкретных аргументов. */

                /* Допустим, нам надо описать класс банковского счёта. Банковский счет должен иметь номер,
                 * однако на момент написания класса может быть неизвестно, какой тип будет представлять номер счета - это может быть числовой номер 1233545,
                 * а может и быть просто набор символов в виде строки, например, "1234878rtyio". На первый взгляд, мы можем просто определить два класса: */
                    class IntAccount {
                    private:
                        int id;
                    public:
                     ...
                     * Шаблоны класса позволяют уменьшить повторяемость кода, задав для класса универсальный тип. Изменим код, применив шаблоны: */
                        template <typename T>
                        class Account {
                        private:
                            T id;
            ...
            // Non-type параметр в шаблоне

                /* Non-type параметр в шаблоне — это специальный параметр шаблона, который заменяется не типом данных, а конкретным значением. Этим значением может быть:
                    * Целочисленное значение (int) или перечисление (enum);
                    * Указатель или ссылка на объект класса;
                    * Указатель или ссылка на функцию;
                    * указатель или ссылка на метод класса;
                    * std::nullptr_t - тип данных, принимающий одно значение - nullptr. */

                // Рассмотрим следующий пример:
                    template <typename T, int size> // size является параметром non-type в шаблоне класса
                    class StaticArray {
                        T array[size];
                    public:
                        T* getArray();
                ...
                // Синтаксис определения шаблона метода и самого метода вне тела класса с параметром non-type:
                    ...
                    T* StaticArray<T, size>::getArray() { return array; }
                 ...
                 * Это связано с тем, что для любого созданного объекта класса StaticArray
                 * его размер является конкретно заданным значением (можно сказать константой), которое передает пользователь. */

            // Полная и частичная специализации

                /* В шаблонах функций мы уже познакомились с понятием "специализация". Так вот, в шаблонах функций эта специализация была всегда полная.
                 * В классах же это понятие расширяется и делится на два подпонятия: полная и частичная специализация.
                     * Полная специализация шаблона позволяет специализировать шаблон класса для работы с определенным типом данных
                     * (или сразу с несколькими типами данных, если есть несколько параметров шаблона). */
                    template <typename T>
                    class A {};

                    template <>
                    class A <int> {}; // Полная специализация шаблона класса

                /* Частичная специализация шаблона класса определяется как некая конфигурация параметров первичного класса,
                 * которая служит аргументом специализированной реализации. Примером такой специализации может служить любой класс-контейнер,
                 * реализованный для хранения объектов-указателей: */
                    template <typename T>
                    class Vector {}; // Шаблон класса

                    template <typename T>
                    class Vector<T*> {}; // Частичная специализация шаблона класса для хранения указателей

            // Использование non-type параметров при специализации

                // Используем описанный выше класс StaticArray.
                    template <typename T, int size>
                    void print(StaticArray<T, size> &array) {
                        for (int count = 0; count < size; ++count)
                            cout << array[count] << ' ';
                    } // Возникнет проблема, если мы захотим печатать массив типа char: между символами будут пробелы.

                // Если мы захотим реализовать полную специализацию, нам придётся отдельно прописывать и точный размер массива, т.к. нам неоткуда будет брать size:
                    template <>
                    void print(StaticArray<char, size> &array) {}; // Будет ошибка, т.к. он не поймёт, что мы хотим подставить вместо <..., int size>

                // Чтобы избежать подобного, используем частичную специализацию шаблона:
                    template <int size>
                    void print(StaticArray<char, size> &array) {};

            // Частичная специализация шаблонов методов

                // Реализуем метод print() в классе StaticArray:
                    template <typename T, int size>
                ...
                // Если, например, мы захотим частично специализировать метод print(), у нас это не получится, т.к. любую функцию нельзя частично специализировать:
                    template <int size>
                    void StaticArray<double, size>::print(); // Произойдёт ошибка

                /* Это можно обойти двумя способами:
                    * 1) Частичная специализация шаблона всего класса: */
                        template <int size>
                    ...
                    /* 2) Использование наследования от шаблона класса, причём функция, которую мы хотели бы специализировать, станет виртуальной: */
                        template <typename T, int size>
                        class StaticArray_Base { // Делаем отдельный родительский класс, иначе не сможем наследоваться от него при специализации
                        protected:
                            T array[size];
                        public:
                            // ...
                        ...
                        public:
                            void print() override {
                                // ...
                            } /* Если используем поля родительского класса, обращаемся к ним через указатель *this,
                                           * т.к. при использовании данного класса родительский класс не будет инстанцирован,
                                           * и мы не сможем явно обратиться к полям из родительского класса. */
                        };
        ...
        { // Псевдонимы
            ...
            /* Объявление псевдонима можно использовать для объявления имени, которое будет использоваться в качестве синонима для ранее объявленного типа.
             * (Этот механизм также называется неофициальным псевдонимом типа). Этот механизм также можно использовать для создания шаблона псевдонима. */

            using UL = unsigned long; // То же самое, что и typedef unsigned long UL

            // Псевдонимы также работают с указателями функций, но гораздо более удобочитаемыми, чем эквивалентное определение типа:
                using func = void (*)(int); // То же самое, что и typedef void (*func)(int)

            /* Отличие typedef механизма от using механизма заключается в том, что typedef не работает с шаблонами.
             * Напротив, синтаксис псевдонима типа в C ++11 позволяет создавать шаблоны псевдонимов: */
                template<typename T> using ptr = T *; // Теперь ptr<T> - это псевдоним для T*
                ptr<int> ptr_int; // То же самое, что и int* ptr_int
        }

        { // Шаблоны с переменным числом аргументов

            // Объявление шаблона с переменным количеством параметров (так называемым "пакетом параметров") выглядит следующим образом:
                template<typename... Types> // Types - пакет параметров
                class Сlassname {};
            ...
            // С помощью определения шаблона класса с пакетом параметров можно также потребовать по крайней мере один параметр:
                template <typename First, typename... Rest> class Сlassname;
            ...
            // Помимо шаблонных классов, можно также объявлять шаблонные функции с переменным количеством параметров. Подобные объявления могут выглядеть следующим образом:
                ...
                // Возможны и другие формы синтаксиса шаблонов функции вариативности:
                    void printf(const char* format, Type&... args);
                    void printf(const char* format, Type&&... args);
                    void printf(const char* format, Type*... args);
                // Описатели, такие как const также, разрешены:
                    void printf(const char* format, const Type... args);

            /* Шаблонные функции с переменным числом аргументов (как и аналогичные шаблонные классы) также могут устанавливать требование о том,
             * что должен быть передан по меньшей мере один параметр: */
                template<typename... First, typename... Rest>
                void printf(const First& first, const Rest&... rest);

            /* В зависимости от того, где именно используется пакет параметров, соответствующим образом интерпретируются элементы этого пакета.
             * Само использование пакета параметров называется "раскрытием пакета", и записывается в коде следующим образом: */
                Types...

            // В функциях мы уже рассмотрели раскрытие пакета, теперь давайте рассмотрим его в классах:
                template<typename... Types>
                class Сlassname: public Types... { // Раскрытие в список базовых классов. 'public Types' - паттерн
                    // ...
                    using OtherClass = OtherClassname<Types...>; // Раскрытие в список параметров другого шаблона. Паттерн - Types.
                    using SomeOtherClass = OtherClassname<Types*...>; //  // Более сложный вариант. Паттерн - Types*.
                    void operator () (Types ... args) { // Раскрытие в список параметров функции. Паттерном является Types, a args - это новый список параметров.
                        foo(&args ...); // Раскрытие в список аргументов при вызове функции
                    }
                    Classname(): Types()... // Раскрытие в списке инициализации в конструкторе.
                };

            // Также в шаблонах с переменным числом аргументов используется оператор sizeof...(), который возвращает количество элементов в пакете параметров.
                template<typename First, typename... Rest>
                void func(const Rest&... args) {
                    constexpr auto num_of_args1(sizeof...(Rest)); // Можно задать num_of_args через Rest...
                    constexpr auto num_of_args2 = sizeof...(args); // Или через args
                    First f_array[num_of_args1];
                }

            // Пример использования шаблонов с переменным числом параметров

                // Хороший способ проиллюстрировать механизм шаблона функции вариативности — использовать его в перезаписи функции вывода:
                    template <typename T> void print(const T& t) {
                        cout << t << endl;
                    }
                    template <typename First, typename... Rest> void print(const First& first, const Rest&... rest) {
                        cout << first << ", ";
                        print(rest...); // Рекурсивный вызов функции, использующий пакет параметров
                    }
                    int main() {
                        print(1); // Вызовется первая функция print()
                        print("first", 2, "third", 4) // Вызовется уже вторая функция print()
                    }
